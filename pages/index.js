  ...
  const printPlanner = (weekDays, weekKey) => {
    ...
    const daysHTML = weekDays.map((d,i)=>{
      ...
      return \`<div style="flex:1;min-width:100px;border-right:1px solid #ddd;padding:0 10px">
        <div style="font-weight:700;color:#1AAB8A;font-size:12px;margin-bottom:4px;text-transform:uppercase">\${DAYS_FULL[i]}</div>
        <div style="font-size:11px;color:#666;margin-bottom:8px">\${d.label}</div>
        <div style="font-size:12px;line-height:1.6;white-space:pre-wrap">\${notes||'—'}</div>
      </div>\`
    }).join('')
    const prios = (meta.prioridades||[]).map(p=>\`<li style="margin:4px 0;font-size:12px;\${p.done?'text-decoration:line-through;color:#999':''}">\${p.text}</li>\`).join('')
    const html = \`<html><head><title>Planner</title><style>
      body{font-family:Arial,sans-serif;padding:30px;color:#1a1a1a;max-width:1000px;margin:0 auto}
      h1{color:#1AAB8A;font-size:18px;margin-bottom:4px} .sub{color:#888;font-size:12px;margin-bottom:20px}
      @media print{body{padding:10px}}
    </style></head><body>
      <h1>📅 Planner Semanal</h1>
      <div class="sub">\${weekDays[0].label} — \${weekDays[6].label}</div>
      <div style="display:flex;gap:0;border:1px solid #ddd;border-radius:8px;overflow:hidden;margin-bottom:20px;padding:14px 10px;min-height:200px">
        \${daysHTML}
      </div>
      \${meta.prioridades?.length?\`<div><h2 style="font-size:13px;color:#333;margin-bottom:8px">⭐ Prioridades</h2><ul>\${prios}</ul></div>\`:''}
      \${meta.importante?\`<div style="margin-top:14px"><h2 style="font-size:13px;color:#333;margin-bottom:6px">⚠ Importante</h2><p style="font-size:12px;line-height:1.6">\${meta.importante}</p></div>\`:''}
    </body></html>\`
    const w=window.open('','_blank'); w.document.write(html); w.document.close(); setTimeout(()=>w.print(),400)
  }
