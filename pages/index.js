...
export default function App(){
  ...
  const [execHistory,setExecHistory]=useState([]) // separate, never overwritten by sync
  ...
  useEffect(()=>{
    ...
    if(sh){
      const d=decShare(sh)
      if(d){
        setCards(d.cards.map(c=>({key:c.k,title:c.t,status:c.s,assignee:c.a})))
        setGmud(d.gmud||{})
        if(d.jiraUrl)setCfg(p=>({...p,jiraUrl:d.jiraUrl}))
        if(d.meetings)setMeetings(d.meetings)
        if(d.checklist)setChecklist(d.checklist)
        setRO(true);setAuthLoading(false);return
      }
    }
    // Check Supabase auth
    const initAuth = async () => {
      ...
      if(all !== null){
        // Supabase respondeu - sempre confiar (mesmo vazio = conta nova sem dados)
        if(all.cfg)      { setCfg(all.cfg); try{localStorage.setItem('gmud-cfg',JSON.stringify(all.cfg))}catch{} }
        if(all.gmud)     { setGmud(all.gmud); try{localStorage.setItem('gmud-gmud',JSON.stringify(all.gmud))}catch{} }
        if(all.cards)    { setCards(all.cards); cardsRef.current=all.cards; try{localStorage.setItem('gmud-cards',JSON.stringify(all.cards))}catch{} }
        if(all.checklist){ setChecklist(all.checklist) }
        if(all.meetings) { setMeetings(all.meetings) }
        if(all.planner)  { setPlanner(all.planner) }
        if(all.history)  { setExecHistory(all.history) }
        if(all.cfg?.jiraUrl) try{localStorage.setItem('gmud-jiraUrl',all.cfg.jiraUrl)}catch{}
      } else {
        // Supabase falhou (sem rede) - fallback para cache local do MESMO usuário
        try{
          ...
          const k=localStorage.getItem('gmud-cards');if(k){const kk=JSON.parse(k);setCards(kk);cardsRef.current=kk}
          ...
          const eh=localStorage.getItem('gmud-history');if(eh)setExecHistory(JSON.parse(eh))
        }catch{}
      }
      // ── One-time migration: push localStorage data to Supabase for any missing keys
      try{
        const localMap = {
          cfg:'gmud-cfg', gmud:'gmud-gmud', cards:'gmud-cards',
          checklist:'gmud-checklist', meetings:'gmud-meetings',
          planner:'gmud-planner', history:'gmud-history'
        }
        const migrated = localStorage.getItem('gmud-migrated-v1')
        if(!migrated){
          const promises = Object.entries(localMap).map(async ([supKey, localKey]) => {
            // Only upload if not already in Supabase
            if(all && all[supKey]) return
            try{
              const raw = localStorage.getItem(localKey)
              if(raw){ const parsed = JSON.parse(raw); await dbSave(supKey, parsed) }
            }catch{}
          })
          await Promise.all(promises)
          // Clear localStorage after migration so next account on same device starts fresh
          const keysToRemove=['gmud-cfg','gmud-gmud','gmud-cards','gmud-checklist','gmud-meetings','gmud-planner','gmud-history','gmud-jiraUrl']
          keysToRemove.forEach(k=>{try{localStorage.removeItem(k)}catch{}})
          localStorage.setItem('gmud-migrated-v1','true')
        }
      }catch{}
      setAuthLoading(false)
      // Auto-sync with Jira if credentials exist
      try{
        ...
        if(url&&saved.email&&saved.token&&saved.project){
          ...
          let savedGmudLocal={}
          try{const gg=localStorage.getItem('gmud-gmud');if(gg)savedGmudLocal=JSON.parse(gg)}catch{}
          try{const kk=localStorage.getItem('gmud-cards');if(kk)cardsRef.current=JSON.parse(kk)}catch{}
          fetch('/api/jira',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jiraUrl:url,email:saved.email,token:saved.token,jql,maxResults:200})})
            .then(r=>r.json()).then(d=>{if(d.issues)proc(d.issues,savedGmudLocal)}).catch(()=>{})
        }
      }catch{}
    }
    initAuth()
  ...
  const persist=useCallback((nc,ng,nf,ncl,nmt)=>{
    try{
      if(nc!==undefined){localStorage.setItem('gmud-cards',JSON.stringify(nc));dbSave('cards',nc)}
      if(ng!==undefined){localStorage.setItem('gmud-gmud',JSON.stringify(ng));dbSave('gmud',ng)}
      if(nf!==undefined){localStorage.setItem('gmud-cfg',JSON.stringify(nf));dbSave('cfg',nf)}
      if(ncl!==undefined){localStorage.setItem('gmud-checklist',JSON.stringify(ncl));dbSave('checklist',ncl)}
      if(nmt!==undefined){localStorage.setItem('gmud-meetings',JSON.stringify(nmt));dbSave('meetings',nmt)}
    }catch{}
  },[])

  const proc=useCallback((issues,gOver)=>{
    const g=gOver??gmud
    const fresh=issues.filter(i=>!HIDDEN.has(i.fields?.status?.name)||g[i.key]?.executed).map(i=>({
      key:i.key,title:i.fields?.summary||'',status:i.fields?.status?.name||'To Do',
      assignee:i.fields?.assignee?.displayName||'—',
      flagged:!!(i.fields?.flagged===true||(Array.isArray(i.fields?.customfield_10021)&&i.fields.customfield_10021.length>0)),
      storyPoints:i.fields?.customfield_10016||i.fields?.customfield_10028||null,
      createdAt:i.fields?.created?i.fields.created.split('T')[0]:'',
      plannedDate:i.fields?.customfield_11299?i.fields.customfield_11299.split('T')[0]:''
    }))
    // Cards state = only active Jira cards. History is stored separately.
    cardsRef.current=fresh
    setCards(fresh);persist(fresh,undefined,undefined,undefined,undefined)
  },[gmud,persist])
  ...
  const toggleDone=(key)=>{
    ...
    const ng={...gmud,[key]:{...(gmud[key]||{}),executed:!cur,executedAt:!cur?today:''}}
    setGmud(ng);persist(undefined,ng,undefined,undefined,undefined)
    if(!cur){
      // Add to separate history - never lost on sync
      const card=cards.find(c=>c.key===key)
      if(card){
        setExecHistory(prev=>{
          const next=[...prev.filter(c=>c.key!==key),{...card,gmudDate:gmud[key]?.date||''}]
          try{localStorage.setItem('gmud-history',JSON.stringify(next));dbSave('history',next)}catch{}
          return next
        })
      }
    }
  ...
  const savePlanner = (newPlanner) => {
    ...
    dbSave('planner', newPlanner)
  }

  const addPlannerItem = (dateKey, text) => {
    if (!text.trim()) return
    const day = planner[dateKey] || {items:[]}
    const items = [...(day.items||[]), {id: Date.now().toString(), text: text.trim(), done: false}]
    savePlanner({...planner, [dateKey]: {...day, items}})
  }
  const togglePlannerItem = (dateKey, id) => {
    const day = planner[dateKey] || {items:[]}
    const items = (day.items||[]).map(i => i.id===id ? {...i, done:!i.done} : i)
    savePlanner({...planner, [dateKey]: {...day, items}})
  }
  const delPlannerItem = (dateKey, id) => {
    const day = planner[dateKey] || {items:[]}
  ...
  const printPlanner = (weekDays, weekKey) => {
    ...
    const daysHTML = weekDays.map((d,i) => {
      const items = planner[d.key]?.items || []
      const itemsHTML = items.length ? items.map(it => '<div style="display:flex;gap:5px;margin:3px 0;font-size:11px' + (it.done?';color:#999':'') + '"><span>' + (it.done?'☑':'☐') + '</span><span style="' + (it.done?'text-decoration:line-through':'') + '">' + it.text + '</span></div>').join('') : '<span style="color:#aaa;font-size:11px">—</span>'
  ...
  const exportCSV=()=>{
    const rows=[['Chave','Título','Status','Responsável','Planejado Jira','Data GMUD','Executada','Data Execução']]
    cards.filter(c=>gmud[c.key]?.date).forEach(c=>{const g=gmud[c.key]||{};rows.push([c.key,c.title,c.status,c.assignee,c.plannedDate||'',g.date||'',g.executed?'Sim':'Não',g.executedAt||''])})
  ...
  const gmudDone=cards.filter(c=>gmud[c.key]?.executed).length
  const gmudLate=cards.filter(c=>gmud[c.key]?.date&&!gmud[c.key]?.executed&&new Date(gmud[c.key].date+'T00:00:00')<now).length
  ...
  // History by month
  // History uses separate store - never overwritten by Jira sync
  const executedCards=execHistory.filter(c=>gmud[c.key]?.date||c.gmudDate)
  const histByMonth=executedCards.reduce((a,c)=>{
    const dk=(gmud[c.key]?.date||c.gmudDate||'').substring(0,7)
    if(!dk)return a
    if(!a[dk])a[dk]=[]
    a[dk].push(c)
    return a
  ...
  const totalSP=visible.reduce((s,c)=>s+(c.storyPoints||0),0)
  const hasSP=totalSP>0
  const avgPerPerson=asgn.length>0?(hasSP?Math.round(totalSP/asgn.length):Math.round(visible.length/asgn.length)):0
  const spLabel=hasSP?'SP médios/responsável':'cards/responsável'
  ...
  const navItems=[
    ...
    {id:'health',   icon:'💚',label:'Saúde do Time'},
      {id:'metricas',  icon:'📈',label:'Métricas Ágeis'},
    ...(!readOnly?[
  ...
  if(authLoading) return <div style={{display:'flex',minHeight:'100vh',background:'#0A0F1E',alignItems:'center',justifyContent:'center',fontFamily:'sans-serif',color:'#475569',fontSize:14,flexDirection:'column',gap:12}}><div style={{width:36,height:36,borderRadius:10,background:'linear-gradient(135deg,#1AAB8A,#10B981)',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontSize:18,fontWeight:700}}>G</div>Carregando...</div>
  return(
  <div style={{display:'flex',minHeight:'100vh',background:T.bg}}>
    ...
    <aside style={{width:220,background:T.sidebar,borderRight:`1px solid ${T.border}`,display:'flex',flexDirection:'column',padding:'20px 0',flexShrink:0,position:'sticky',top:0,height:'100vh',overflowY:'auto'}}>
      ...
      <div style={{padding:'12px 10px 0',display:'flex',flexDirection:'column',gap:6}}>
        ...
        {user&&(
          <div style={{borderTop:`1px solid ${T.border}`,paddingTop:10,marginTop:4}}>
            <div style={{fontSize:11,color:T.ter,marginBottom:6,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{user.email}</div>
            <button onClick={async()=>{
  const keys=['gmud-cfg','gmud-gmud','gmud-cards','gmud-checklist','gmud-meetings','gmud-planner','gmud-history','gmud-jiraUrl','gmud-migrated-v1']
  keys.forEach(k=>{try{localStorage.removeItem(k)}catch{}})
  await supabase.auth.signOut()
  window.location.href='/login'
}}
              style={{width:'100%',padding:'8px 10px',borderRadius:8,border:`1px solid rgba(239,68,68,.3)`,background:'rgba(239,68,68,.08)',color:'#F87171',cursor:'pointer',fontSize:12,fontWeight:500,display:'flex',alignItems:'center',gap:8}}>
              🚪 Sair
            </button>
          </div>
        )}
    ...
    <main style={{flex:1,padding:'28px 28px',overflowY:'auto',minWidth:0}}>
          ...
          <Card style={{padding:'16px 20px'}}>
            ...
            <div style={{overflowX:'auto'}}>
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
                <thead><tr>{['Chave','Título','Status','Responsável','Planejado Jira','Data GMUD',...(!readOnly?['✓']:['Situação'])].map(h=><th key={h} style={{textAlign:'left',padding:'7px 10px',fontSize:10,fontWeight:700,color:T.ter,borderBottom:`1px solid ${T.border}`,textTransform:'uppercase',letterSpacing:'.5px',whiteSpace:'nowrap'}}>{h}</th>)}</tr></thead>
                <tbody>
                  {sorted.map(c=>{
                    const g=gmud[c.key]||{},done=!!g.executed,isLate=g.date&&!done&&new Date(g.date+'T00:00:00')<now
                    return<tr key={c.key} style={{background:isLate?'rgba(239,68,68,.04)':g.date?'rgba(26,171,138,.04)':'transparent'}}>
                      <td style={{padding:'10px 10px',borderBottom:`1px solid ${T.border2}`,whiteSpace:'nowrap'}}>
                        <CardLink k={c.key} jiraUrl={cfg.jiraUrl} style={{fontSize:12,color:isLate?'#F87171':T.teal}}/>
                        {isLate&&<span style={{fontSize:9,marginLeft:4,color:'#F87171'}}>ATRASADA</span>}
                      </td>
                      <td style={{padding:'10px 10px',borderBottom:`1px solid ${T.border2}`,maxWidth:240,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',color:T.text}} title={c.title}>{c.title}</td>
                      ...
                      <td style={{padding:'10px 10px',borderBottom:`1px solid ${T.border2}`,textAlign:'center'}}>
                        ...
                        <button onClick={()=>toggleDone(c.key)} title='Marcar executada — some desta lista' style={{width:30,height:30,borderRadius:8,border:`1.5px solid ${T.border}`,background:'transparent',color:T.ter,cursor:'pointer',fontSize:15,display:'flex',alignItems:'center',justifyContent:'center',transition:'all .15s'}}>✓</button>}
                      </td>
                    </tr>
                  })}
                </tbody>
                    ...
                    <tbody>
                      {histByMonth[curHistMonth].map(c=>{const g=gmud[c.key]||{};return(
                        <tr key={c.key}>
                          ...
                          <td style={{padding:'10px 10px',borderBottom:`1px solid ${T.border2}`,fontSize:12,color:T.sec}}>{fmt(g.date||c.gmudDate)}</td>
                          <td style={{padding:'10px 10px',borderBottom:`1px solid ${T.border2}`,fontSize:12,color:'#34D399',fontWeight:600}}>{fmt(g.executedAt)||fmt(g.date||c.gmudDate)}</td>
                        </tr>
                      )})}
                    </tbody>
                  </table>
                ...
                {weekDays.map((d,i)=>{
                  ...
                  return(
                    <div key={d.key} style={{display:'flex',flexDirection:'column',gap:0}}>
                      ...
                      <div style={{border:`1px solid ${isToday?T.teal:T.border}`,borderRadius:'0 0 8px 8px',background:T.card,padding:'8px',minHeight:220,display:'flex',flexDirection:'column',gap:4}}>
                        {(planner[d.key]?.items||[]).map(item=>(
                          <div key={item.id} style={{display:'flex',alignItems:'flex-start',gap:6}}>
                            <button onClick={()=>togglePlannerItem(d.key,item.id)} style={{flexShrink:0,width:16,height:16,marginTop:2,borderRadius:4,border:`1.5px solid ${item.done?T.teal:T.ter}`,background:item.done?T.teal:'transparent',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontSize:10,transition:'all .15s',padding:0}}>
                              {item.done?'✓':''}
                            </button>
                            <span style={{fontSize:12,color:item.done?T.ter:T.text,textDecoration:item.done?'line-through':'none',flex:1,lineHeight:1.5,wordBreak:'break-word'}}>{item.text}</span>
                            <button onClick={()=>delPlannerItem(d.key,item.id)} style={{flexShrink:0,background:'transparent',border:'none',color:T.ter,cursor:'pointer',fontSize:11,padding:'0 2px',opacity:0,transition:'opacity .15s'}} className="del-btn">✕</button>
                          </div>
                        ))}
                        <input
                          placeholder="+ Adicionar tarefa..."
                          style={{background:'transparent',border:'none',borderTop:(planner[d.key]?.items||[]).length>0?`1px dashed ${T.border}`:'none',marginTop:(planner[d.key]?.items||[]).length>0?4:0,paddingTop:(planner[d.key]?.items||[]).length>0?6:0,color:T.ter,fontSize:12,outline:'none',width:'100%',fontFamily:'inherit'}}
                          onKeyDown={e=>{if(e.key==='Enter'&&e.target.value.trim()){addPlannerItem(d.key,e.target.value);e.target.value=''}}}
                        />
                      </div>
                    </div>
                  )
            ...
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              ...
              <Card style={{padding:'14px 16px'}}>
                ...
                {[
                  ...
                  {label:'Tarefas concluídas', value:(()=>{const all=weekDays.flatMap(d=>planner[d.key]?.items||[]);return all.filter(i=>i.done).length+'/'+all.length})(), color:'#34D399'},
                ].map(m=>(
      ...
      {/* ── MÉTRICAS ÁGEIS ── */}
      {tab==='metricas'&&!readOnly&&(()=>{
        const now = new Date()
        const daysSince = ds => ds ? Math.floor((now - new Date(ds+'T00:00:00'))/864e5) : null
        const WIP_ST = new Set(['Em Desenvolvimento','In Progress','Em teste','Testing','Code Review','Em Refinamento Técnico','Aguardando Testes','Em Homologação','Aguardando Homologação','Aguardando deploy'])

        // ── Deployment Frequency (GMUDs executadas por semana - últimas 8 semanas)
        const weeks8 = Array.from({length:8},(_,i)=>{
          const ws = new Date(now); ws.setDate(now.getDate()-(7*(7-i))); ws.setHours(0,0,0,0)
          const we = new Date(ws); we.setDate(ws.getDate()+6)
          const count = execHistory.filter(c=>{
            const d=gmud[c.key]?.executedAt||gmud[c.key]?.date||c.gmudDate
            if(!d)return false; const dt=new Date(d+'T00:00:00'); return dt>=ws&&dt<=we
          }).length
          return {name:ws.toLocaleDateString('pt-BR',{day:'2-digit',month:'short'}),value:count}
        })
        const deployFreq = weeks8.length ? (weeks8.reduce((s,w)=>s+w.value,0)/8).toFixed(1) : 0

        // ── Lead Time real (createdAt → executedAt)
        const realLeadTimes = execHistory.filter(c=>c.createdAt&&(gmud[c.key]?.executedAt||gmud[c.key]?.date)).map(c=>{
          const created  = new Date(c.createdAt+'T00:00:00')
          const executed = new Date((gmud[c.key]?.executedAt||gmud[c.key]?.date)+'T00:00:00')
          return Math.max(0,Math.floor((executed-created)/864e5))
        })
        const avgLeadTime = realLeadTimes.length ? Math.round(realLeadTimes.reduce((a,b)=>a+b,0)/realLeadTimes.length) : null

        // ── Lead Time GMUD (agendamento → execução)
        const gmudLeadTimes = execHistory.filter(c=>gmud[c.key]?.date&&(gmud[c.key]?.executedAt)).map(c=>{
          const sched = new Date(gmud[c.key].date+'T00:00:00')
          const exec  = new Date(gmud[c.key].executedAt+'T00:00:00')
          return Math.floor((exec-sched)/864e5)
        })
        const avgGmudDays = gmudLeadTimes.length ? (gmudLeadTimes.reduce((a,b)=>a+b,0)/gmudLeadTimes.length).toFixed(1) : null

        // ── Taxa de atraso: usa data planejada do Jira (customfield_11299) como referência
        const late = execHistory.filter(c=>{
          const planned = c.plannedDate  // Data Planejada Entrega Produção do Jira
          const exec    = gmud[c.key]?.executedAt||gmud[c.key]?.date||c.gmudDate
          if(!planned||!exec) return false
          return new Date(exec+'T00:00:00') > new Date(planned+'T00:00:00')
        }).length
        const lateWithPlanned = execHistory.filter(c=>c.plannedDate).length
        const lateRate = lateWithPlanned > 0 ? Math.round((late/lateWithPlanned)*100) : null

        // ── WIP atual
        const wipCards = visible.filter(c=>WIP_ST.has(c.status))
        const wip = wipCards.length

        // ── Cards envelhecendo (sem GMUD, com createdAt)
        const aging = visible.filter(c=>!gmud[c.key]?.date&&c.createdAt).map(c=>({...c,age:daysSince(c.createdAt)})).filter(c=>c.age!=null).sort((a,b)=>b.age-a.age)
        const oldCards30 = aging.filter(c=>c.age>=30).length

        // ── Gargalos: status com mais cards
        const statusByCount = Object.entries(stCounts).filter(([s])=>!['Concluído','Em Produção','Done'].includes(s)).sort((a,b)=>b[1]-a[1]).slice(0,6)

        // ── Velocity (SP ou cards por mês)
        const hasSP = execHistory.some(c=>c.storyPoints>0)
        const vel6m = Array.from({length:6},(_,i)=>{
          const d=new Date(now.getFullYear(),now.getMonth()-(5-i),1)
          const mo=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`
          const items=execHistory.filter(c=>(gmud[c.key]?.executedAt||gmud[c.key]?.date||'').startsWith(mo))
          return{name:d.toLocaleDateString('pt-BR',{month:'short',year:'2-digit'}),value:hasSP?items.reduce((s,c)=>s+(c.storyPoints||0),0):items.length}
        })

        // ── KPI color helpers
        const kpiColor = (v,good,warn) => v==null?T.ter:v<=good?'#34D399':v<=warn?'#FCD34D':'#F87171'
        const rateColor = (v,good,warn) => v==null?T.ter:v>=good?'#34D399':v>=warn?'#FCD34D':'#F87171'

        const KPI = ({icon,label,value,unit,color,explain,sub}) => (
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:'16px 18px',display:'flex',flexDirection:'column',gap:6}}>
            <div style={{display:'flex',alignItems:'center',gap:7}}>
              <span style={{fontSize:18}}>{icon}</span>
              <span style={{fontSize:10,fontWeight:700,color:T.ter,textTransform:'uppercase',letterSpacing:'.5px'}}>{label}</span>
            </div>
            <div style={{fontSize:30,fontWeight:700,color:color||T.text,lineHeight:1}}>
              {value!=null?value:<span style={{fontSize:15,color:T.ter}}>—</span>}
              {value!=null&&unit&&<span style={{fontSize:13,fontWeight:400,color:T.sec,marginLeft:5}}>{unit}</span>}
            </div>
            {sub&&<div style={{fontSize:11,color:T.sec}}>{sub}</div>}
            <div style={{fontSize:11,color:T.ter,borderTop:`1px solid ${T.border2}`,paddingTop:7,marginTop:2,lineHeight:1.5}}>{explain}</div>
          </div>
        )
        ...
        <div>
          <div style={{marginBottom:22}}>
            <h1 style={{fontSize:22,fontWeight:700}}>📈 Métricas Ágeis</h1>
            <p style={{fontSize:13,color:T.ter,marginTop:3}}>{execHistory.length} GMUDs executadas · {visible.length} cards ativos</p>
          </div>

          {execHistory.length===0&&visible.length===0&&(
            <Card style={{textAlign:'center',padding:'40px',color:T.ter}}>
              <div style={{fontSize:36,marginBottom:12}}>📊</div>
              <div>Conecte o Jira e execute GMUDs para ver as métricas</div>
            </Card>
          )}

          {/* ── KPIs principais ── */}
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:12,marginBottom:20}}>
            <KPI icon="🚀" label="Frequência de Deploy"
              value={deployFreq>0?deployFreq:null} unit="deploys/semana"
              color="#60A5FA"
              explain="Quantas GMUDs são executadas por semana em média. Meta DORA elite: várias por dia." />
            <KPI icon="⏱️" label="Lead Time Real"
              value={avgLeadTime} unit="dias"
              color={kpiColor(avgLeadTime,14,30)}
              sub={avgLeadTime?`Baseado em ${realLeadTimes.length} cards`:null}
              explain="Tempo médio da criação do card até a execução em produção. Meta: abaixo de 14 dias." />
            <KPI icon="📅" label="Tempo até Deploy"
              value={avgGmudDays} unit="dias"
              color={kpiColor(parseFloat(avgGmudDays),0,7)}
              explain="Dias entre o agendamento da GMUD e a execução real. Negativo = adiantado." />
            <KPI icon="⚠️" label="Taxa de Atraso"
              value={lateRate!=null?`${lateRate}%`:null}
              color={rateColor(100-(lateRate||0),80,60)}
              sub={lateRate!=null?`${late} de ${lateWithPlanned} com data planejada`:null}
              explain="% de GMUDs executadas depois da data agendada. Meta: abaixo de 20%." />
            <KPI icon="🔄" label="WIP Atual"
              value={wip} unit="cards"
              color={wip>10?'#F87171':wip>6?'#FCD34D':'#34D399'}
              sub={`${Math.round((wip/Math.max(visible.length,1))*100)}% do total ativo`}
              explain="Cards atualmente em desenvolvimento/teste. WIP alto = gargalo e contexto fragmentado." />
            <KPI icon="👴" label="Cards Envelhecendo"
              value={oldCards30} unit="cards"
              color={oldCards30>5?'#F87171':oldCards30>2?'#FCD34D':'#34D399'}
              sub={aging.length>0?`Maior: ${aging[0]?.age||0} dias sem GMUD`:null}
              explain="Cards sem GMUD agendada há mais de 30 dias. Risco de débito técnico e esquecimento." />
          </div>

          {/* ── Gráficos ── */}
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:20}}>
            <Card>
              <SectionTitle>🚀 Frequência de Deploy — últimas 8 semanas</SectionTitle>
              <ResponsiveContainer width="100%" height={160}>
                <BarChart data={weeks8} margin={{top:4}}>
                  <XAxis dataKey="name" tick={{fontSize:10,fill:T.ter}} axisLine={false} tickLine={false}/>
                  <YAxis tick={{fontSize:10,fill:T.ter}} allowDecimals={false} axisLine={false} tickLine={false}/>
                  <TT/>
                  <Bar dataKey="value" name="Deploys" fill="#60A5FA" radius={[4,4,0,0]}/>
                </BarChart>
              </ResponsiveContainer>
              <div style={{fontSize:11,color:T.ter,marginTop:8}}>Cada barra = 1 semana. Ideal: consistente e crescendo.</div>
            </Card>

            <Card>
              <SectionTitle>{hasSP?'⚡ Velocity em Story Points':'⚡ Velocity em cards'} — últimos 6 meses</SectionTitle>
              <ResponsiveContainer width="100%" height={160}>
                <BarChart data={vel6m} margin={{top:4}}>
                  <XAxis dataKey="name" tick={{fontSize:10,fill:T.ter}} axisLine={false} tickLine={false}/>
                  <YAxis tick={{fontSize:10,fill:T.ter}} allowDecimals={false} axisLine={false} tickLine={false}/>
                  <TT/>
                  <Bar dataKey="value" name={hasSP?'Story Points':'Cards'} fill={T.teal} radius={[4,4,0,0]}/>
                </BarChart>
              </ResponsiveContainer>
              <div style={{fontSize:11,color:T.ter,marginTop:8}}>{hasSP?'Configure Story Points no Jira para dados precisos.':'Adicione Story Points no Jira para medir capacidade real.'}</div>
            </Card>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:20}}>
            <Card>
              <SectionTitle>🚧 Gargalos por Status</SectionTitle>
              <div style={{display:'flex',flexDirection:'column',gap:8}}>
                {statusByCount.length===0&&<div style={{color:T.ter,fontSize:13}}>Sem dados</div>}
                {statusByCount.map(([s,v])=>{
                  const c=gsc(s), pct=Math.round((v/Math.max(visible.length,1))*100)
                  return(
                    <div key={s}>
                      <div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}>
                        <span style={{fontSize:12,color:c.tx,fontWeight:600,display:'flex',alignItems:'center',gap:5}}>
                          <span style={{width:7,height:7,borderRadius:'50%',background:c.dot,display:'inline-block'}}/>
                          {s}
                        </span>
                        <span style={{fontSize:12,color:T.ter}}>{v} cards ({pct}%)</span>
                      </div>
                      <div style={{height:5,borderRadius:3,background:T.card2,overflow:'hidden'}}>
                        <div style={{height:'100%',width:`${pct}%`,background:c.dot,borderRadius:3,transition:'width .4s'}}/>
                      </div>
                    </div>
                  )
                })}
              </div>
              <div style={{fontSize:11,color:T.ter,marginTop:10,borderTop:`1px solid ${T.border2}`,paddingTop:8}}>Status com mais cards = possível gargalo no fluxo.</div>
            </Card>

            <Card>
              <SectionTitle>👴 Cards Sem GMUD (mais antigos)</SectionTitle>
              {aging.length===0
                ? <div style={{color:T.ter,fontSize:13}}>{visible.filter(c=>!gmud[c.key]?.date).length} cards sem GMUD, mas sem data de criação disponível. Sincronize o Jira.</div>
                : <>
                    <div style={{display:'flex',flexDirection:'column',gap:6,maxHeight:200,overflowY:'auto'}}>
                      {aging.slice(0,8).map(c=>(
                        <div key={c.key} style={{display:'flex',alignItems:'center',gap:8,padding:'6px 8px',background:T.card2,borderRadius:7}}>
                          <CardLink k={c.key} jiraUrl={cfg.jiraUrl} style={{fontSize:11,fontWeight:700,color:c.age>=30?'#F87171':c.age>=14?'#FCD34D':T.teal,whiteSpace:'nowrap'}}/>
                          <span style={{fontSize:11,color:T.sec,flex:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}} title={c.title}>{c.title}</span>
                          <span style={{fontSize:11,fontWeight:600,color:c.age>=30?'#F87171':c.age>=14?'#FCD34D':'#34D399',whiteSpace:'nowrap'}}>{c.age}d</span>
                        </div>
                      ))}
                    </div>
                    <div style={{fontSize:11,color:T.ter,marginTop:8}}>🔴 +30 dias · 🟡 14-30 dias · 🟢 menos de 14 dias</div>
                  </>
              }
            </Card>
          </div>

          {/* ── Nota sobre Cycle Time ── */}
          <Card>
            <SectionTitle>ℹ️ Sobre Cycle Time e outras métricas avançadas</SectionTitle>
            <p style={{fontSize:13,color:T.sec,lineHeight:1.7}}>
              O <strong style={{color:T.text}}>Cycle Time</strong> (tempo em desenvolvimento até entrega) e o <strong style={{color:T.text}}>Flow Efficiency</strong> requerem o histórico de transições de status do Jira (Changelog API), que vai além dos dados atuais.
              O <strong style={{color:T.text}}>Lead Time Real</strong> acima usa a data de criação do card, disponível após a próxima sincronização com o Jira.
            </p>
          </Card>
        </div>
        )
