...
function CardLink({k, jiraUrl, style={}, extraStyle={}}){
  const s={fontWeight:700,...style,...extraStyle}
  if(jiraUrl) return<a href={`${jiraUrl}/browse/${k}`} target="_blank" rel="noreferrer" style={{color:'inherit',textDecoration:'none',...s}}>{k}</a>
  return<span style={s}>{k}</span>
}
          ...
          return<div key={day} style={{minHeight:80,background:isToday?'rgba(26,171,138,.1)':T.card2,borderRadius:8,padding:'6px',border:`1px solid ${isToday?T.teal:T.border2}`}}>
            ...
            {events.slice(0,3).map(e=>{
              ...
              return<div key={e.key} onClick={()=>onCardClick&&onCardClick(e)} title={`${e.key}: ${e.title}`}
                ...
                {e.gmud.executed?'✓ ':''}<CardLink k={e.key} jiraUrl={cfg.jiraUrl} style={{color:'inherit'}}/>
              </div>
            })}
