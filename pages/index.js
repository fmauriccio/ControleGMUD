...
export default function App(){
  ...
  useEffect(()=>{
    ...
    // Note: executed cards are preserved in localStorage and merged on sync
  },[])
  ...
  const proc=useCallback((issues,gOver)=>{
    ...
    const fresh=issues.filter(i=>!HIDDEN.has(i.fields?.status?.name)||g[i.key]?.executed).map(i=>({
      ...
      flagged:!!(i.fields?.flagged||(i.fields?.customfield_10021?.length>0)||(i.fields?.customfield_10021)),
      storyPoints:i.fields?.customfield_10016||i.fields?.customfield_10028||i.fields?.story_points||null
    }))
    // Preserve executed cards not in current sync (moved to Em Produção etc.)
    const freshKeys=new Set(fresh.map(c=>c.key))
    const preserved=cards.filter(c=>g[c.key]?.executed&&!freshKeys.has(c.key))
  ...
  // Story points average per person
  const totalSP=visible.reduce((s,c)=>s+(c.storyPoints||0),0)
  const avgPerPerson=asgn.length>0?(totalSP>0?Math.round(totalSP/asgn.length):Math.round(visible.length/asgn.length)):0
  const spLabel=totalSP>0?'SP/responsável':'cards/responsável'
