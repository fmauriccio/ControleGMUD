  ...
  window.location.href='/login'
    ...
    <main style={{flex:1,padding:'28px 28px',overflowY:'auto',minWidth:0}}>
      ...
      {tab==='metricas'&&!readOnly&&(()=>{
        ...
        // ── Throughput: cards executados por semana (últimas 8 semanas)
        const throughputWeeks = Array.from({length:8},(_,i)=>{
          ...
          const lbl = weekStart.toLocaleDateString('pt-BR',{day:'2-digit',month:'short'})
          return {name:lbl, value:count}
        })
        const avgThroughput = throughputWeeks.length ? (throughputWeeks.reduce((s,w)=>s+w.value,0)/throughputWeeks.filter(w=>w.value>0).length||0).toFixed(1) : 0

        // ── Velocity: SP ou cards por mês (últimos 6 meses)
        const hasSPData = execHistory.some(c=>c.storyPoints>0)
        const velocityMonths = Array.from({length:6},(_,i)=>{
          const d = new Date(now.getFullYear(), now.getMonth()-(5-i), 1)
          const mo = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`
          const items = execHistory.filter(c=>{
            const exDate = gmud[c.key]?.executedAt||gmud[c.key]?.date||c.gmudDate||''
            return exDate.startsWith(mo)
          })
          const value = hasSPData ? items.reduce((s,c)=>s+(c.storyPoints||0),0) : items.length
          return {name:d.toLocaleDateString('pt-BR',{month:'short',year:'2-digit'}), value}
        })
        const avgVelocity = velocityMonths.filter(m=>m.value>0).length
          ? (velocityMonths.reduce((s,m)=>s+m.value,0)/velocityMonths.filter(m=>m.value>0).length).toFixed(1) : 0

        // ── Lead Time: dias entre agendamento e execução
        const leadTimes = execHistory.filter(c=>gmud[c.key]?.date&&(gmud[c.key]?.executedAt||c.gmudDate)).map(c=>{
          const sched = new Date((gmud[c.key]?.date||c.gmudDate)+'T00:00:00')
          const exec  = new Date((gmud[c.key]?.executedAt||gmud[c.key]?.date||c.gmudDate)+'T00:00:00')
          return Math.abs(Math.ceil((exec-sched)/864e5))
        }).filter(d=>d>=0)
        const avgLeadTime = leadTimes.length ? (leadTimes.reduce((a,b)=>a+b,0)/leadTimes.length).toFixed(1) : null

        // ── Taxa de entrega no prazo
        const onTime = execHistory.filter(c=>{
          const sched = gmud[c.key]?.date
          const exec  = gmud[c.key]?.executedAt||gmud[c.key]?.date
          if(!sched||!exec) return false
          return new Date(exec+'T00:00:00') <= new Date(sched+'T00:00:00')
        }).length
        const onTimePct = execHistory.length ? Math.round((onTime/execHistory.length)*100) : null


        const MetricBox = ({title, value, unit, color, explain, warn}) => (
          <div style={{background:T.card,border:`1px solid ${warn?'rgba(248,113,113,.3)':T.border}`,borderRadius:12,padding:'16px 18px'}}>
            <div style={{fontSize:10,fontWeight:700,color:T.ter,textTransform:'uppercase',letterSpacing:'.5px',marginBottom:6}}>{title}</div>
            <div style={{fontSize:28,fontWeight:700,color:color||T.text,lineHeight:1,marginBottom:6}}>
              {value??<span style={{color:T.ter,fontSize:16}}>Sem dados</span>}
              {value!=null&&unit&&<span style={{fontSize:13,color:T.sec,marginLeft:5,fontWeight:400}}>{unit}</span>}
            </div>
            <div style={{fontSize:11,color:T.ter,lineHeight:1.5,borderTop:`1px solid ${T.border2}`,paddingTop:8,marginTop:4}}>{explain}</div>
          </div>
        )
        ...
        <div>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:12}}>
            <div>
              ...
              <p style={{fontSize:13,color:T.ter,marginTop:2}}>Baseadas nas GMUDs executadas · {execHistory.length} no total</p>
            </div>

          </div>

          {execHistory.length===0&&(
            <div style={{...card,textAlign:'center',padding:'40px',color:T.ter}}>
              <div style={{fontSize:36,marginBottom:12}}>📊</div>
              <div>Execute GMUDs na aba Cards / GMUDs para ver as métricas</div>
            </div>
          )}
          ...
          <>
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:14,marginBottom:20}}>
              <MetricBox
                title="Throughput"
                value={avgThroughput>0?avgThroughput:null}
                unit="cards/semana"
                color="#60A5FA"
                explain="Quantidade de itens entregues por semana. Mede a cadência de entrega do time."
              />
              <MetricBox
                title={hasSPData?"Velocity (SP)":"Velocity (cards)"}
                value={avgVelocity>0?avgVelocity:null}
                unit={hasSPData?"SP/mês":"cards/mês"}
                color={T.teal}
                explain={hasSPData
                  ? "Story Points entregues por mês. Indica a capacidade de trabalho do time em pontos."
                  : "Cards entregues por mês. Configure Story Points no Jira para uma medição mais precisa."}
              />
              <MetricBox
                title="Lead Time"
                value={avgLeadTime}
                unit={avgLeadTime?"dias":null}
                color={avgLeadTime>7?'#F87171':'#34D399'}
                warn={avgLeadTime>7}
                explain="Tempo médio entre o agendamento da GMUD e a execução. Ideal: abaixo de 7 dias."
              />
              <MetricBox
                title="Entrega no Prazo"
                value={onTimePct!=null?`${onTimePct}%`:null}
                color={onTimePct>=80?'#34D399':onTimePct>=60?'#FCD34D':'#F87171'}
                warn={onTimePct!=null&&onTimePct<60}
                explain="% de GMUDs executadas na data agendada ou antes. Meta recomendada: acima de 80%."
              />
            </div>

            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:20}}>
              <Card>
                <SectionTitle>Throughput — últimas 8 semanas</SectionTitle>
                <ResponsiveContainer width="100%" height={160}>
                  <BarChart data={throughputWeeks} margin={{top:4}}>
                    ...
                    <TT/>
                    <Bar dataKey="value" name="Cards" fill="#60A5FA" radius={[4,4,0,0]}/>
                  </BarChart>
                </ResponsiveContainer>
              </Card>
              <Card>
                <SectionTitle>Velocity — últimos 6 meses {hasSPData?'(SP)':'(cards)'}</SectionTitle>
                <ResponsiveContainer width="100%" height={160}>
                  <BarChart data={velocityMonths} margin={{top:4}}>
                    <XAxis dataKey="name" tick={{fontSize:10,fill:T.ter}} axisLine={false} tickLine={false}/>
                    <YAxis tick={{fontSize:10,fill:T.ter}} allowDecimals={false} axisLine={false} tickLine={false}/>
                    <TT/>
                    <Bar dataKey="value" name={hasSPData?"Story Points":"Cards"} fill={T.teal} radius={[4,4,0,0]}/>
                  </BarChart>
                </ResponsiveContainer>
              </Card>
            </div>
            ...
            <Card style={{marginBottom:20}}>
              <SectionTitle>Sobre o Cycle Time</SectionTitle>
              <p style={{fontSize:13,color:T.sec,lineHeight:1.6}}>
                O <strong style={{color:T.text}}>Cycle Time</strong> (tempo desde o início do desenvolvimento até a entrega) requer acesso ao histórico de transições de status do Jira (Changelog API), que vai além dos dados atuais. Para habilitá-lo futuramente, seria necessário registrar a data em que cada card entrou em "Em Desenvolvimento" — algo que pode ser adicionado numa próxima versão.
              </p>
            </Card>
