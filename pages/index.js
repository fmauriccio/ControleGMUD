    ...
    <main style={{flex:1,padding:'28px 28px',overflowY:'auto',minWidth:0}}>
      ...
      {tab==='metricas'&&!readOnly&&(()=>{
        ...
        const late = execHistory.filter(c=>{
          const sched=gmud[c.key]?.date, exec=gmud[c.key]?.executedAt
          if(!sched||!exec)return false
          return new Date(exec+'T00:00:00') > new Date(sched+'T00:00:00')
        }).length
        const lateRate = execHistory.length ? Math.round((late/execHistory.length)*100) : null
        ...
        <div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:12,marginBottom:20}}>
            ...
            <KPI icon="⚠️" label="Taxa de Atraso"
              ...
              sub={lateRate!=null?`${late} de ${execHistory.length} GMUDs`:null}
