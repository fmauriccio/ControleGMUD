...
export default function App(){
  ...
  useEffect(()=>{
    ...
    // Auto-sync: read EVERYTHING from localStorage directly (state not yet populated)
    try{
      ...
      if(url&&saved.email&&saved.token&&saved.project){
        ...
        // Pre-load cardsRef AND gmud from localStorage (state not yet populated at this point)
        let savedGmudLocal = {}
        try{
          const kk=localStorage.getItem('gmud-cards')
          if(kk) cardsRef.current=JSON.parse(kk)
          const gg=localStorage.getItem('gmud-gmud')
          if(gg) savedGmudLocal=JSON.parse(gg)
        }catch{}
  ...
  const proc=useCallback((issues,gOver)=>{
    ...
    // Use ref (always current) to preserve executed cards not returned by Jira
    const freshKeys=new Set(fresh.map(c=>c.key))
    const preserved=cardsRef.current.filter(c=>g[c.key]?.executed&&!freshKeys.has(c.key))
    const p=[...fresh,...preserved]
    cardsRef.current=p
