...
// ─── Calendar ─────────────────────────────────────────────────────────────────
function CalendarView({cards,gmud,onCardClick}){
    ...
    <Card style={{padding:'16px'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4}}>
        {cells.map((day,i)=>{
          ...
          return<div key={day} style={{minHeight:80,background:isToday?'rgba(26,171,138,.1)':T.card2,borderRadius:8,padding:'6px',border:`1px solid ${isToday?T.teal:T.border2}`}}>
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    if(sh){
      const d=decShare(sh)
      if(d){setCards(d.cards.map(c=>({key:c.k,title:c.t,status:c.s,assignee:c.a})));setGmud(d.gmud||{});if(d.jiraUrl)setCfg(p=>({...p,jiraUrl:d.jiraUrl}));setRO(true);return}
    }
    try{
  ...
  const shareLink=()=>{
    const enc=encShare(cards,gmud,cfg.jiraUrl)
  ...
  <div style={{display:'flex',minHeight:'100vh',background:T.bg}}>
    ...
    <main style={{flex:1,padding:'28px 28px',overflowY:'auto',minWidth:0}}>
                  ...
                  {sorted.map(c=>{
                    ...
                    return<tr key={c.key} style={{background:done?'transparent':isLate?'rgba(239,68,68,.04)':g.date?'rgba(26,171,138,.04)':'transparent',opacity:done?.45:1,transition:'all .2s'}}>
                      <td style={{padding:'10px 10px',borderBottom:`1px solid ${T.border2}`,whiteSpace:'nowrap'}}>
                        {cfg.jiraUrl?<a href={`${cfg.jiraUrl}/browse/${c.key}`} target="_blank" rel="noreferrer" style={{fontSize:12,fontWeight:700,color:done?T.ter:isLate?'#F87171':T.teal,textDecoration:done?'line-through':'none'}}>{c.key}</a>:<span style={{fontSize:12,fontWeight:700,color:done?T.ter:isLate?'#F87171':T.teal,textDecoration:done?'line-through':'none'}}>{c.key}</span>}
                      ...
                      <td style={{padding:'10px 10px',borderBottom:`1px solid ${T.border2}`,whiteSpace:'nowrap'}}><Badge s={c.status}/></td>
      ...
      {/* ── CALENDÁRIO ── */}
      {tab==='calendar'&&<CalendarView cards={visible} gmud={gmud}/>}
                  ...
                  <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
                    ...
                    <tbody>
                      {histByMonth[curHistMonth].map(c=>{const g=gmud[c.key]||{};return(
                        <tr key={c.key}>
                          <td style={{padding:'10px 10px',borderBottom:`1px solid ${T.border2}`}}>{cfg.jiraUrl?<a href={`${cfg.jiraUrl}/browse/${c.key}`} target="_blank" rel="noreferrer" style={{fontWeight:700,color:T.teal,fontSize:12}}>{c.key}</a>:<span style={{fontWeight:700,color:T.teal,fontSize:12}}>{c.key}</span>}</td>
                ...
                <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:8}}>
                  <div>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
                      <span style={{fontSize:13,fontWeight:700,color:T.teal}}>{c.key}</span>
                      <Badge s={c.status}/>
                      <span style={{fontSize:11,padding:'2px 8px',borderRadius:5,fontWeight:600,background:dt.bg,color:dt.tx}}>{dt.lbl}</span>
                    </div>
                ...
                <div style={{display:'grid',gap:6}}>
                  {CHECKLIST_DEFAULTS.map(item=>(
                    <label key={item.id} style={{display:'flex',alignItems:'center',gap:10,cursor:'pointer',padding:'6px 8px',borderRadius:7,background:cl[item.id]?'rgba(52,211,153,.08)':'transparent',transition:'background .15s'}}>
                      <input type="checkbox" checked={!!cl[item.id]} onChange={()=>toggleCheck(c.key,item.id)} style={{width:16,height:16,accentColor:'#34D399',cursor:'pointer'}}/>
      ...
      {tab==='meetings'&&(
        <>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:10}}>
            ...
            <Btn primary onClick={()=>{setNewMeeting(true);setSelMeeting(null)}}>+ Nova reunião</Btn>
          </div>
            ...
            {selMeeting?(()=>{
              ...
              return<Card>
                <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:8}}>
                  <div>
                    <input value={m.title} onChange={e=>updMeeting(m.id,{title:e.target.value})} style={{...inpSt,fontSize:16,fontWeight:700,background:'transparent',border:'none',padding:'0',color:T.text,width:'100%',marginBottom:4}} placeholder="Título da reunião"/>
                  ...
                  <div style={{display:'flex',gap:8}}>
                    ...
                    <Btn small danger onClick={()=>delMeeting(m.id)}>🗑</Btn>
                  </div>
                </div>
                ...
                <div style={{marginBottom:16}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}>
                    ...
                    <Btn small onClick={()=>addItem(m.id,'item')}>+ Adicionar</Btn>
                  </div>
                  ...
                  {(m.items||[]).map(it=>(
                    <div key={it.id} style={{display:'flex',alignItems:'center',gap:8,marginBottom:6}}>
                      ...
                      <button onClick={()=>delItem(m.id,'item',it.id)} style={{background:'transparent',border:'none',color:T.ter,cursor:'pointer',fontSize:14,padding:'4px'}}>✕</button>
                    </div>
                  ))}
                </div>
                ...
                <div style={{marginBottom:16}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}>
                    ...
                    <Btn small onClick={()=>addItem(m.id,'action')}>+ Adicionar</Btn>
                  </div>
                  ...
                  {(m.actions||[]).map(ac=>(
                    <div key={ac.id} style={{display:'flex',alignItems:'center',gap:8,marginBottom:6}}>
                      ...
                      <input value={ac.text} onChange={e=>updItem(m.id,'action',ac.id,{text:e.target.value})} style={{...inpSt,flex:2,padding:'6px 10px',fontSize:13,textDecoration:ac.done?'line-through':'none',color:ac.done?T.ter:T.text}} placeholder="Ação a executar..."/>
                      <input value={ac.owner} onChange={e=>updItem(m.id,'action',ac.id,{owner:e.target.value})} style={{...inpSt,width:110,padding:'6px 10px',fontSize:12}} placeholder="Responsável"/>
                      <button onClick={()=>delItem(m.id,'action',ac.id)} style={{background:'transparent',border:'none',color:T.ter,cursor:'pointer',fontSize:14,padding:'4px'}}>✕</button>
                    </div>
                  ))}
                </div>
                ...
                <div>
                  ...
                  <textarea style={{...taSt,minHeight:80}} placeholder="Anotações livres, decisões tomadas, observações importantes..." value={m.notes||''} onChange={e=>updMeeting(m.id,{notes:e.target.value})}/>
                </div>
              </Card>
