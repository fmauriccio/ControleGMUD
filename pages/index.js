...
// ─── Calendar ─────────────────────────────────────────────────────────────────
function CalendarView({cards,gmud,onCardClick,jiraUrl}){
...
export default function App(){
  ...
  const [refForm,setRefForm]=useState({title:'',type:'fibonacci',custom:''})
  ...
  const startRefinamento = async () => {
    if (!refForm.title.trim()) return
    const customValues = refForm.type==='custom' ? refForm.custom.split(',').map(s=>s.trim()).filter(Boolean) : []
    const s = await createSession('poker',{type:refForm.type,title:refForm.title,customValues:JSON.stringify(customValues)})
    setRefSession(s)
  }
  ...
  const navItems=[
    ...
    ...(!readOnly?[
      ...
      {id:'refinamento', icon:'🃏',label:'Refinamento'},
  ...
  <div style={{display:'flex',minHeight:'100vh',background:T.bg}}>
    ...
    <main style={{flex:1,padding:'28px 28px',overflowY:'auto',minWidth:0}}>
      ...
      {/* ── CALENDÁRIO ── */}
      {tab==='calendar'&&<CalendarView cards={visible} gmud={gmud} jiraUrl={cfg.jiraUrl}/>}
      ...
      {/* ── REFINAMENTO ── */}
      {tab==='refinamento'&&!readOnly&&(
        <div>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:10}}>
            <div><h1 style={{fontSize:22,fontWeight:700}}>🃏 Refinamento</h1><p style={{fontSize:13,color:T.ter,marginTop:2}}>Planning Poker — crie uma sessão e compartilhe com o time</p></div>
          </div>
          {!refSession?(
            <Card style={{maxWidth:520}}>
              <SectionTitle>Nova sessão de votação</SectionTitle>
              <div style={{display:'flex',flexDirection:'column',gap:12}}>
                <div>
                  <label style={{fontSize:11,fontWeight:700,color:T.ter,textTransform:'uppercase',letterSpacing:'.4px',display:'block',marginBottom:5}}>Título / História</label>
                  <input style={{...inpSt,width:'100%'}} placeholder="Ex: PROJ-101 - Implementar SSO" value={refForm.title} onChange={e=>setRefForm(p=>({...p,title:e.target.value}))} onKeyDown={e=>e.key==='Enter'&&startRefinamento()}/>
                </div>
                <div>
                  <label style={{fontSize:11,fontWeight:700,color:T.ter,textTransform:'uppercase',letterSpacing:'.4px',display:'block',marginBottom:5}}>Tipo de votação</label>
                  <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                    {[['fibonacci','Fibonacci (0,1,2,3,5,8...)'],['tshirt','T-Shirts (XS,S,M,L...)'],['custom','Personalizado']].map(([v,l])=>(
                      <button key={v} onClick={()=>setRefForm(p=>({...p,type:v}))} style={{padding:'8px 14px',borderRadius:8,border:`1px solid ${refForm.type===v?T.teal:T.border}`,background:refForm.type===v?'rgba(26,171,138,.12)':'transparent',color:refForm.type===v?T.teal:T.sec,cursor:'pointer',fontSize:13,fontWeight:refForm.type===v?600:400}}>{l}</button>
                    ))}
                  </div>
                </div>
                {refForm.type==='custom'&&(
                  <div>
                    <label style={{fontSize:11,fontWeight:700,color:T.ter,textTransform:'uppercase',letterSpacing:'.4px',display:'block',marginBottom:5}}>Valores (separados por vírgula)</label>
                    <input style={{...inpSt,width:'100%'}} placeholder="Ex: P, M, G, GG, ?" value={refForm.custom} onChange={e=>setRefForm(p=>({...p,custom:e.target.value}))}/>
                  </div>
                )}
                <Btn primary onClick={startRefinamento} disabled={!refForm.title.trim()}>▶ Criar sessão</Btn>
              </div>
            </Card>
          ):(
            <div style={{display:'grid',gap:16}}>
              <Card style={{border:`1px solid rgba(26,171,138,.3)`}}>
                <SectionTitle>Sessão ativa — {refSession.title}</SectionTitle>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:16}}>
                  {[
                    {label:'Link para participantes',url:`${typeof window!=='undefined'?window.location.origin:''}/refinamento?session=${refSession.id}`,copy:true},
                    {label:'Link do host (você)',url:`${typeof window!=='undefined'?window.location.origin:''}/refinamento?session=${refSession.id}&host=1`,copy:true},
                  ].map(({label,url,copy})=>(
                    <div key={label}>
                      <div style={{fontSize:11,color:T.ter,marginBottom:4}}>{label}</div>
                      <div style={{display:'flex',gap:6}}>
                        <input readOnly value={url} style={{...inpSt,fontSize:11,flex:1}}/>
                        {copy&&<Btn small onClick={()=>navigator.clipboard.writeText(url)}>📋</Btn>}
                      </div>
                    </div>
                  ))}
                </div>
                <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                  <Btn onClick={()=>window.open(`/refinamento?session=${refSession.id}&host=1`,'_blank')} primary>🚀 Abrir sessão</Btn>
                  <Btn onClick={()=>setRefSession(null)}>+ Nova sessão</Btn>
                </div>
              </Card>
            </div>
          )}
